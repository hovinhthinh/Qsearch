...
public class TableQfactSaver {
    public static void main(String[] args) {
        String wikiFile = "/local/home/hvthinh/TabQs/data/wiki+tablem/enwiki-20200301-pages-articles-multistream.xml.bz2.tables+id_annotation+linking.gz";
        String tablemFile = "/GW/D5data-11/hvthinh/TABLEM/all/all+id.annotation+linking.gz";

        double LINKING_THRESHOLD = 0.70;

        Gson gson = new Gson();
        PrintWriter out = FileUtils.getPrintWriter("/GW/D5data-12/hvthinh/TabQs/annotation+linking/wiki+tablem.gz", "UTF-8");
        for (String file : Arrays.asList(wikiFile, tablemFile))
                ...
                for (int qCol = 0; qCol < table.nColumn; ++qCol) {
                    ...
                    for (int row = 0; row < table.nDataRow; ++row) {
                        ...
                        out.println(String.format("%s\t%s\t%s\t%s\t%s\t%s",
                                ...
                                table.source.replace("WIKIPEDIA:Link:", "").replace("TABLEM:Link:", "")
                        ));
                    }
                }
            }
